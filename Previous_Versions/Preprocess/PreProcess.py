    ...
    for idx in reversed(inputFile.index):
        ...
        if iteration != 1:
            ...
            if (newUserNumber - oldUserNumber) == 0:
                ...
                holeSize = diff[0] * 365 * 24 * 60 + diff[1] * 30 * 24 * 60 + diff[2] * 24 * 60 + diff[3] * 60 + diff[
                    4]  # wrong action
                if (holeSize <= (180 * 24)) and (holeSize >= gapThreshold):
                    gapSize = holeSize

                    # Calculate session actions
                    print(list(inputFile.loc[19098861:19098858, "artname"]))
                    sessionActionsList = list(inputFile.loc[beginSessionIndex:endSessionIndex, "artname"])
